      ...
      if (sender != (*i))
        {
          ...
          // --------------------- Modification
          Ptr<NetDevice> srcNetDevice = sender->GetDevice ();
          uint32_t srcNode = srcNetDevice->GetNode ()->GetId ();

          MyTag tagCopy;
          if (packet->PeekPacketTag(tagCopy)) { // for future use
            uint32_t sNode=tagCopy.GetSNode();
            uint32_t rNode=tagCopy.GetRNode();
            
            if (rNode != dstNode || sNode != srcNode) {
              //NS_FATAL_ERROR("FDFDFD > Node different in tag and method" << dstNode << "/" << rNode << " or " << sNode << " / " << srcNode) ;
              NS_FATAL_ERROR("FDFDFD > Node different in tag ("<< dstNode<<"|"<<  srcNode <<") and method ("<< rNode<<"|"<<  sNode <<")" << "->"<< dstNode << "/" << rNode << " or " << sNode << " / " << srcNode) ;
            //std::cout<<"*******************FDFDFD > Node different in tag ("<< dstNode<<"|"<<  srcNode <<") and method ("<< rNode<<"|"<<  sNode <<")" << "->"<< dstNode << "/" << rNode << " or " << sNode << " / " << srcNode<<std::endl ;
            
            }
            
            NS_LOG_UNCOND ("+AtPhy " << Simulator::Now().GetSeconds() << " s"
                           << " packet " << packet->GetUid()
                           << " send from node " << srcNode << " to " << dstNode
                           << " txPower: " << txPowerDbm << " dbm"
                           << " rxPower: " << rxPowerDbm << " dbm"
                           << " distance: " << senderMobility->GetDistanceFrom (receiverMobility) << " m"
                           << " delay: " << delay << " s") ;
          }
          else {
